...
class HdpDriver(object):
    """
    Class to create corpora, HDP models, and perform queries on the models.
    ...
    def __init__(self, **kwargs):
        """
        Initialize HdpDriver object.

        HdpDriver object has a corpus and hdp model either created here or loaded from file.
        ...
        # If make_hdp flag is True, train a new HDP.
        if kwargs['make_hdp']:
            self.hdp = models.HdpModel(self.cor, id2word=self.cor.dictionary,)
            # Save HDP model
            self.hdp.save('../../data/save/driver/{}_hdp.model'.format(self.corpus_type))

        # load corpus from file
        self.cor = corpora.MmCorpus('../../data/save/driver/{}_corpus.mm'.format(self.corpus_type))
        self.cor.dictionary = corpora.Dictionary.load('../../data/save/driver/{}_dictionary.dict'
                                                      .format(self.corpus_type))

        # Load HDP model
        self.hdp = models.HdpModel.load('../../data/save/driver/{}_hdp.model'.format(self.corpus_type))
        ...
        ven_p_dists = [self.hdp[self.cor[i]] for i in ven_offsets]
        self.ven_p_dists_dense = matutils.corpus2dense(ven_p_dists, int(self.num_topics), int(len(ven_offsets)))
        # self.ven_p_dists_dense = matutils.corpus2dense(ven_p_dists, int(self.num_topics))
    ...
    def __str__(self):
        ...
        :return: str repr of the HdpDriver
        :rtype: str
        """
        out = u'\nCorpus:\n'
        out += u'   Tokenizer: {}\n'.format(self.corpus_type)
    ...
    def print_topics(self, n=10):
        ...
        for i, topic in enumerate(self.hdp.show_topics(self.num_topics, n, log=True, formatted=False)):
            print(u'\nTOPIC {0}:'.format(i))
    ...
    def compare_venues(self, venues):
        ...
        ven_p_dists = [self.hdp.get_document_topics(self.cor[i]) for i in ven_offsets]
    ...
    def docbows_to_hellinger_matrix(self, bow_corpus):
        ...
        """
        size = len(bow_corpus)
        hdp_cor = [self.hdp.get_document_topics(doc_bow) for doc_bow in bow_corpus]
        hdp_cor_matrix = matutils.corpus2dense(hdp_cor, self.num_topics, size)
        return hellinger_matrix(hdp_cor_matrix)
...
if __name__ == '__main__':
    driver_settings = {'corpus_type':'twokenize',
                       ...
                       'make_corpus':False,
                       'make_hdp':False}
    driver = HdpDriver(**driver_settings)
    ...
    # driver.vis_super_MDS(super_dist_matrix)
